...
const CartOrderConfirmation = () => {
  ...
  // Use ccBuilderItemDitelsList for items
  const itemsList = order.ccBuilderItemDitelsList || [];
  ...
  return (
    <div className="p-4 sm:p-8 max-w-4xl mx-auto">
      <h2 className="text-2xl font-bold mb-6">Cart Order Confirmation</h2>
      ...
      <div className="border p-4 sm:p-6 rounded-md bg-gray-50">
                ...
                <div key={index} className="flex gap-4 border-b pb-4">
                  ...
                  <div className="flex-1">
                    <h5 className="font-medium text-sm sm:text-base">
                      {item.name || item.ccBuilder?.name || item.item?.name || "N/A"}
                    </h5>
                    <p className="text-sm text-gray-600">
                      ID: {item.id || "N/A"}
                    </p>
                    ...
                    <p className="text-sm font-semibold">
                      Subtotal: Tk{" "}
                      {((item.specialprice || item.regularprice || 0) * (item.quantity || 1)).toFixed(2)}
                    </p>
                    {item.ccBuilder && (
                      <p className="text-sm text-gray-600">
                        Category: {item.ccBuilder.name || "N/A"}
                      </p>
                    )}
                  </div>
                </div>
        ...
        <div className="mt-4">
          ...
          <p className="text-sm text-gray-600">
            ...
            {order.user?.name || profile?.name || user?.name || "Guest"}
          </p>
          <p className="text-sm text-gray-600">
            <span className="font-medium">Email:</span>{" "}
            {order.user?.email || profile?.email || user?.email || "N/A"}
          </p>
          <p className="text-sm text-gray-600">
            <span className="font-medium">Phone:</span>{" "}
            {order.user?.phoneNo || profile?.phoneNo || user?.phoneNo || "Not provided"}
          </p>
        </div>
      </div>
      ...
      <div className="mt-6 flex justify-end gap-4">
        <button
          className="bg-gray-300 text-black px-4 py-2 rounded hover:bg-gray-400 text-sm font-medium"
          onClick={() => navigate("/")}
