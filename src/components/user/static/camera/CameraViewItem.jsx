...
const CameraViewItem = () => {
  ...
  // Set main image when currentCamera changes
  useEffect(() => {
    if (currentCamera?.imagea) {
      setMainImage(currentCamera.imagea);
    } else {
  ...
  const galleryImages = useMemo(() => {
    ...
    return [
      currentCamera.imagea,
      currentCamera.imageb,
      currentCamera.imagec,
    ]
      .filter(Boolean)
  ...
  const handlePlaceOrderClick = useCallback(() => {
    if (!user?.id || !profile?.email || !token) {
      ...
      navigate("/login", { state: { from: `/camera/${id}` } });
      return;
    }
    navigate("/checkout", {
      state: {
  ...
  }, [user, profile, token, currentCamera, quantity, navigate, id]);
    ...
    <Box sx={{ p: 4, display: "flex", flexDirection: { xs: "column", md: "row-reverse" }, gap: 4 }}>
      ...
      <Box sx={{ flex: 1, px: { xs: 2, md: 4 } }}>
        ...
        <Typography variant="body2" color="text.secondary">
          Product ID: {currentCamera.id}
        </Typography>
        <Typography variant="h6" color="error" fontWeight="bold" sx={{ mt: 1 }}>
          Special Price: Tk {currentCamera.specialprice}
        </Typography>
        <Typography variant="body1" color="text.secondary" sx={{ mt: 1 }}>
          Regular Price: Tk {currentCamera.regularprice}
        </Typography>
        {currentCamera.specialprice < currentCamera.regularprice && (
          <Typography variant="body2" color="secondary" sx={{ mt: 1 }}>
            Save Tk {currentCamera.regularprice - currentCamera.specialprice} on online order
          </Typography>
        )}
        ...
        <ul className="list-disc pl-6 text-sm text-gray-600">
          {currentCamera.details &&
            currentCamera.details.split(", ").map((detail, index) => (
