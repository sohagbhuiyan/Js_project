...
const NetworkViewItem = () => {
  ...
  // Set main image when currentNetwork changes
  useEffect(() => {
    if (currentNetwork?.imagea) {
      setMainImage(currentNetwork.imagea);
    } else {
  ...
  const galleryImages = useMemo(() => {
    ...
    return [
      currentNetwork.imagea,
      currentNetwork.imageb,
      currentNetwork.imagec,
    ]
      .filter(Boolean)
  ...
  const handlePlaceOrderClick = useCallback(() => {
    if (!user?.id || !profile?.email || !token) {
      ...
      navigate("/login", { state: { from: `/network/${id}` } });
      return;
    }
    navigate("/checkout", {
      state: {
  ...
  }, [user, profile, token, currentNetwork, quantity, navigate, id]);
    ...
    <Box sx={{ p: 4, display: "flex", flexDirection: { xs: "column", md: "row-reverse" }, gap: 4 }}>
      ...
      <Box sx={{ flex: 1, px: { xs: 2, md: 4 } }}>
        ...
        <Typography variant="body2" color="text.secondary">
          Product ID: {currentNetwork.id}
        </Typography>
        <Typography variant="h6" color="error" fontWeight="bold" sx={{ mt: 1 }}>
          Special Price: Tk {currentNetwork.specialprice}
        </Typography>
        <Typography variant="body1" color="text.secondary" sx={{ mt: 1 }}>
          Regular Price: Tk {currentNetwork.regularprice}
        </Typography>
        {currentNetwork.specialprice < currentNetwork.regularprice && (
          <Typography variant="body2" color="secondary" sx={{ mt: 1 }}>
            Save Tk {currentNetwork.regularprice - currentNetwork.specialprice} on online order
          </Typography>
        )}
        ...
        <ul className="list-disc pl-6 text-sm text-gray-600">
          {currentNetwork.details &&
            currentNetwork.details.split(", ").map((detail, index) => (
              <li key={index}>{detail}</li>
            ))}
          {currentNetwork.datatransferrate && (
            <li>Data Transfer Rate: {currentNetwork.datatransferrate}</li>
          )}
        </ul>
      ...
      <Box sx={{ display: "flex", px: 5, py:2, flexDirection: { xs: "column-reverse", md: "row" }, gap: 2 }}>
        ...
        >
          <img
            src={mainImage}
            alt={currentNetwork.name || "Network Device"}
...
export default NetworkViewItem;
